...
class IoT
{
     ...
     * @param object $DB
     */
    public static function init(object $DB): void
    {
        self::$DB = $DB;
    }
     ...
     * @param string $mac
     */
    public static function getDeviceId(string $mac): ?int
    {
        $result = self::$DB->query('SELECT id FROM devices WHERE mac="' . $mac . '"');
    ...
    {
        if ($value == '-127.00' or $value == '85.00' or $value == 'nan') {
            return;
        }
     ...
     * @return array
     */
    public static function getRelaysState($id): array
    {
