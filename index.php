							...
							<?php foreach($arSensors as $arSensor): ?>
                                ...
                                <div class="card" id="displaySensor_<?php echo $arSensor['sensor']; ?>">
                                    <div class="card-header bg-cyan bg-inverse custom_text_center">
                                        <h4><i class="<?php if(stristr($arSensor['sensor'], 'dht_h')){ echo 'fa fa-flask'; } else { echo 'ion-thermometer'; } ?>"></i> Датчик <?php echo strtoupper(str_replace('ds', 'DS18B20_', $arSensor['sensor'])); ?></h4>
                                        <ul class="card-actions">
                                            <li>
                                                <button type="button" onclick="App.cards('#displaySensor_<?php echo $arSensor['sensor']; ?>', 'content_toggle' );"><i class="ion-ios-toggle"></i></button>
                                            </li>
                                        </ul>
                                    </div>
                                    <div class="card-block">
                                        <p>
	                                        <div class="monitoring_value"><span id="<?php echo $arSensor['sensor'];?>"><?php echo $arSensor['value'];?></span> <?php if(stristr($arSensor['sensor'], 'dht_h')){ echo '%'; } else { echo 'C'; } ?></div>
	                                    </p>
                                    </div>
                                </div>
                            </div>
									...
									$(document).ready(function(){
										setInterval(function(){
											$.ajax({
											  ...
											  data: {deviceId: "<?php echo $USER['deviceId']; ?>", sensor: "<?php echo $arSensor['sensor']; ?>"},
	...
	<div class="col-sm-6 col-lg-6">
		<div class="card" id="graphCard_<?php echo $arSensor['sensor'];?>">
			<div class="bg-inverse bg-purple card-header">
				<h4><i class="ion-stats-bars"></i> История показаний датчика <?php echo strtoupper(str_replace('ds', 'DS18B20_', $arSensor['sensor'])); ?></h4>
				<ul class="card-actions">
                    <li>
                        <button type="button" onclick="App.cards('#graphCard_<?php echo $arSensor['sensor'];?>', 'content_toggle' );"><i class="ion-ios-toggle"></i></button>
                    </li>
                </ul>
			</div>
			<div class="card-block">
